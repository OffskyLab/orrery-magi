...
final class SpecFinalizeCommandTests: XCTestCase {
    ...
    private func runFinalize(sessionId: String, exitCode: Int32) throws {
        var cmd = OrreryMagi.SpecFinalizeCommand()
    ...
    private func seedRunningState(
        ...
        try SpecRunStateWriter.write(sessionId: sessionId, state: state)
        return sessionId
    }
    ...
    func testIdempotency_terminalStateIsNotOverwritten() throws {
        ...
        try SpecRunStateWriter.update(sessionId: sid) { s in
            s.status = "done"
            s.completedAt = "2026-04-22T00:05:00Z"
            s.lastError = "prior-final-state"
        }
        let before = try SpecRunStateReader.load(sessionId: sid)
    ...
    func testExitCode0_mapsToDone() throws {
        ...
        let state = try SpecRunStateReader.load(sessionId: sid)
    ...
    func testExitCodeNonZero_mapsToFailed_withLastError() throws {
        ...
        let stderrPath = SpecRunStateReader.stderrLogPath(sessionId: sid).path
    ...
    func testProgressLog_populatesCompletedStepsAndFailedStep() throws {
        ...
        let progressPath = SpecRunStateReader.progressLogPath(sessionId: sid).path
        let lines = [
