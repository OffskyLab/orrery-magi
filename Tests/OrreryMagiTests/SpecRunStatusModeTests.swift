...
/// dispatch. Uses the built `.build/debug/orrery-magi` binary as a subprocess
...
final class SpecRunStatusModeTests: XCTestCase {
    ...
    /// Locate the `.build/debug/orrery-magi` binary.
    private func orreryBinaryPath() throws -> String {
        // When run via `swift test`, CWD is the repo root.
        let cwd = FileManager.default.currentDirectoryPath
        let candidate = "\(cwd)/.build/debug/orrery-magi"
        guard FileManager.default.isExecutableFile(atPath: candidate) else {
            throw XCTSkip("debug orrery-magi binary not built at \(candidate) — run `swift build` first")
        }
        return candidate
    }
