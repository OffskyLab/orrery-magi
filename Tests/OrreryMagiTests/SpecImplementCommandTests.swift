...
final class SpecImplementCommandTests: XCTestCase {
    ...
    private let fixturePath = "Tests/OrreryMagiTests/Fixtures/minimal-implement-spec.md"
    ...
    private func orreryBinary() throws -> String {
        let candidate = "\(FileManager.default.currentDirectoryPath)/.build/debug/orrery-magi"
        guard FileManager.default.isExecutableFile(atPath: candidate) else {
            throw XCTSkip("debug orrery-magi not built — run `swift build` first")
        }
        return candidate
    }
