...
final class SpecImplementRunnerTests: XCTestCase {
    ...
    func testBuildWrapperShell_containsTimeoutWatchdog_whenTimeoutPositive() {
        let wrapper = OrreryMagi.SpecImplementRunner.buildWrapperShell(
    ...
    func testShellQuote_escapesEmbeddedSingleQuote() {
        let quoted = OrreryMagi.SpecImplementRunner.shellQuote("don't")
    ...
    func testResolveOrreryBinaryPath_returnsAbsolute_orFallback() {
        let path = OrreryMagi.SpecImplementRunner.resolveOrreryBinaryPath()
        // Either it's absolute (and exists), or it's "orrery-magi" (PATH fallback).
        let isAbsExisting = path.hasPrefix("/") &&
            FileManager.default.isExecutableFile(atPath: path)
        let isFallback = (path == "orrery-magi")
        XCTAssertTrue(isAbsExisting || isFallback,
                      "resolved path should be abs/executable OR literal 'orrery-magi'; got: \(path)")
        // Crucially, never returns a relative path like ".build/debug/orrery-magi"
        if path != "orrery-magi" {
    ...
    func testRun_specNotFound_throws() {
        XCTAssertThrowsError(try OrreryMagi.SpecImplementRunner.run(
    ...
    func testRun_specMissingMandatoryHeading_throws_noStateWritten() throws {
        ...
        let rootDir = SpecRunStateReader.rootDir
    ...
    func testRun_resumeWithDelegateId_presence_inWrapperViaStateCheck() throws {
        ...
        prior.status = "done"
        prior.completedAt = "2026-04-21T00:10:00Z"
        try SpecRunStateWriter.write(sessionId: sid, state: prior)
        ...
        do {
            _ = try OrreryMagi.SpecImplementRunner.run(
                specPath: fixturePath,
                tool: nil,
                environment: nil,
                store: .default,
        ...
        let after = try SpecRunStateReader.load(sessionId: sid)
