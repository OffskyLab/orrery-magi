...
public struct SpecVerifyRunner {
    ...
    private static func invokeMagiReview(
        specPath: String,
        environment: String?,
        store: EnvironmentStore
    ) -> ReviewOutcome {
        ...
        do {
            let availableTools = Tool.allCases.filter(isToolAvailable)
            guard availableTools.count >= 2 else {
                return ReviewOutcome(
                    verdict: .advisoryOnly,
                    reasoning: "magi review unavailable: insufficient available tools",
                    flaggedItems: []
                )
            }
            ...
            let output = MagiOrchestrator.generateReport(run: run)
