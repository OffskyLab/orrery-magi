...
public struct SpecFinalizeCommand: ParsableCommand {
    ...
    private func finalize() throws {
        ...
        guard SpecRunStateReader.exists(sessionId: sessionId) else {
            warn("_spec-finalize called for unknown session=\(sessionId); skipping")
            return
        }
        var state = try SpecRunStateReader.load(sessionId: sessionId)
        ...
        let progressPath = SpecRunStateReader.progressLogPath(sessionId: sessionId).path
